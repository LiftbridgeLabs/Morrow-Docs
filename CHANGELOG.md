...
- Offline downloads that keep going in the background (and, for single-file books, even if you fully quit the app), cancel one in progress with a swipe, plus an automatic playback cache with user-set size limit and cellular protection (nothing downloads over cellular unless enabled). Progress made while listening offline is queued and pushed to your server the instant you're back online, so switching devices afterward picks up from where you actually left off
